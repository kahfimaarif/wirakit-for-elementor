(function () {
    ...
    var GET_ALL_ACCESS_URL = data.getAllAccessUrl || 'https://wiratheme.com/contact-us/';
    ...
    var state = {
        ...
        templateImporter: {
            ...
            disableElementorDefaultColors: false,
            disableElementorDefaultFonts: false,
            remoteLoading: false,
            remoteLoaded: false,
            remoteError: '',
            remoteKits: [],
    ...
    function renderWidgetsView() {
        ...
        var modules = Array.isArray(data.moduleItems) ? data.moduleItems : [];
        ...
        return [
            ...
            renderGroup('Modules', 'modules', modules),
            widgetGroupsHtml,
            '</div>',
            '</div>'
        ].join('');
    ...
    function renderDashboardView() {
        return [
            ...
            '<span>Version ' + escapeHtml(data.pluginVersion || '1.0.2') + '</span>',
            '</div>',
    ...
    function renderTemplateBuilderView() {
        ...
        return [
            ...
            '<button type="button" class="wkit-template-builder-tab accent-text ', activeTab === 'wkit-header' ? 'is-active' : '', '" data-tb-tab="wkit-header"><span class="dashicons dashicons-table-row-after margin-right-7"></span>Header Builder</button>',
            '<button type="button" class="wkit-template-builder-tab accent-text ', activeTab === 'wkit-footer' ? 'is-active' : '', '" data-tb-tab="wkit-footer"><span class="dashicons dashicons-table-row-before margin-right-7"></span>Footer Builder</button>',
            '<button type="button" class="wkit-template-builder-tab accent-text ', activeTab === 'wkit-single' ? 'is-active' : '', '" data-tb-tab="wkit-single"><span class="dashicons dashicons-admin-page margin-right-7"></span>Single Builder</button>',
            '<button type="button" class="wkit-template-builder-tab accent-text ', activeTab === 'wkit-archive' ? 'is-active' : '', '" data-tb-tab="wkit-archive"><span class="dashicons dashicons-category margin-right-7"></span>Archive Builder</button>',
            '<button type="button" class="wkit-template-builder-tab accent-text ', activeTab === 'wkit-search' ? 'is-active' : '', '" data-tb-tab="wkit-search"><span class="dashicons dashicons-search margin-right-7"></span>Search Result Builder</button>',
            '<button type="button" class="wkit-template-builder-tab accent-text ', activeTab === 'wkit-404' ? 'is-active' : '', '" data-tb-tab="wkit-404"><span class="dashicons dashicons-warning margin-right-7"></span>404 Builder</button>',
            '</div>',
            '</div>',
            '</div>',
            '</div>',
    ...
    function isAdvancedTemplateImportEnabled() {
        return !!(state.settings && state.settings.modules && state.settings.modules['advanced-template-import']);
    }

    function renderTemplateImporterTabButtons() {
        var activeTab = state.templateImporter.activeTab;
        ...
        return [
            ...
            '<button type="button" class="wkit-template-builder-tab accent-text ', activeTab === 'imported-template' ? 'is-active' : '', '" data-ti-tab="imported-template"><span class="dashicons dashicons-format-aside margin-right-7"></span>Imported Template Kits</button>',
    ...
    function renderTemplateImporterFullImportTab() {
        return [
            ...
            '</div>',
            '<div class="wkit-filter-group">',
            '<div class="wkit-radio-inputs" role="radiogroup" aria-label="Pricing filter">',
            '<label class="wkit-radio">',
            '<input type="radio" name="wkit-pricing-full-import" value="all" data-ti-filter-full-import="1" ' + ((state.templateImporter.fullImportPricingFilter || 'all') === 'all' ? 'checked' : '') + '>',
            '<span class="name">All</span>',
            '</label>',
            '<label class="wkit-radio">',
            '<input type="radio" name="wkit-pricing-full-import" value="free" data-ti-filter-full-import="1" ' + ((state.templateImporter.fullImportPricingFilter || 'all') === 'free' ? 'checked' : '') + '>',
            '<span class="name">Free</span>',
            '</label>',
            '<label class="wkit-radio">',
            '<input type="radio" name="wkit-pricing-full-import" value="subscribe" data-ti-filter-full-import="1" ' + ((state.templateImporter.fullImportPricingFilter || 'all') === 'subscribe' ? 'checked' : '') + '>',
    ...
    function renderTemplateImporterFullImportModal() {
        ...
        var pluginRows = requiredPlugins.map(function (plugin) {
            ...
            return [
                ...
                '<strong>' + escapeHtml(plugin.name || plugin.slug || plugin.file || 'Plugin') + '</strong>',
        ...
        return [
            ...
            canClose && awaitingPluginStep
                ? '<button type="button" class="button wkit-button-warning" data-ti-full-import-install="1" ' + (state.templateImporter.installingRequiredPlugins ? 'disabled' : '') + '>' + (state.templateImporter.installingRequiredPlugins ? 'Checking...' : 'Apply & Refresh') + '</button>'
                : '',
            canClose && awaitingPluginStep
    ...
    function openInstallRequiredPluginsModal() {
        ...
        state.templateImporter.disableElementorDefaultColors = !!(state.templateImporter.elementorSettings && state.templateImporter.elementorSettings.disable_default_colors);
        state.templateImporter.disableElementorDefaultFonts = !!(state.templateImporter.elementorSettings && state.templateImporter.elementorSettings.disable_default_fonts);
        render();
    }
    ...
    function renderInstallRequiredPluginsModal() {
        ...
        return [
            ...
            state.templateImporter.installingRequiredPlugins ? 'Checking...' : 'Apply & Refresh',
            '</button>',
            '</div>',
            '</div>',
            '</div>'
    ...
    function renderTemplateImporterBrowseGrid() {
        ...
        return [
            '<div class="wkit-settings-group-grid template-kit">',
            kits.map(function (kit) {
                var importBusy = state.templateImporter.actionLoadingId === ('import-remote-' + String(kit.slug || ''));
                var isSubscribeOnly = String(kit && kit.pricing_type ? kit.pricing_type : '').toLowerCase() === 'subscribe';
                var canInstallSubscribe = !isSubscribeOnly || state.extendedLicenseActive;
                return [
                    '<div class="wkit-setting-item row-box">',
                    isSubscribeOnly ? '<span class="wkit-premium-ribbon"><span>Premium</span></span>' : '',
                    kit.thumbnail ? '<div class="wkit-ti-kit-thumb"><img src="' + escapeHtml(kit.thumbnail) + '" alt="' + escapeHtml(kit.name || kit.slug || 'Demo Kit') + '"></div>' : '<div class="wkit-ti-kit-thumb wkit-ti-kit-thumb-empty"></div>',
                    '<div class="wkit-tk-wrapper">',
                    '<div class="template-kit-imported">',
                    ...
                    canInstallSubscribe
                        ? '<button type="button" class="button wkit-button-primary" data-ti-import-remote="' + escapeHtml(kit.slug || '') + '" ' + (importBusy ? 'disabled' : '') + '>' + (importBusy ? 'Importing...' : 'Import Template Kit') + '</button>'
                        : (
                            '<a class="button wkit-button-tertiary" href="https://wiratheme.com/contact-us/" target="_blank" rel="noopener noreferrer"><span class="dashicons dashicons-awards margin-right-7"></span>Get all Access</a>'
                        ),
                    '</div>',
                    '</div>',
                    '</div>'
    ...
    function renderTemplateImporterFullImportBrowseGrid() {
        ...
        return [
            '<div class="wkit-settings-group-grid template-kit">',
            kits.map(function (kit) {
                var importBusy = state.templateImporter.actionLoadingId === ('import-full-remote-' + String(kit.slug || ''));
                var isSubscribeOnly = String(kit && kit.pricing_type ? kit.pricing_type : '').toLowerCase() === 'subscribe';
                var canInstallSubscribe = !isSubscribeOnly || state.extendedLicenseActive;
                return [
                    '<div class="wkit-setting-item row-box">',
                    ...
                    kit.demo ? '<a class="button wkit-button-secondary" href="' + escapeHtml(kit.demo) + '" target="_blank" rel="noopener noreferrer">Demo</a>' : '',
                    canInstallSubscribe
                        ? '<button type="button" class="button wkit-button-primary" data-ti-full-import-remote="' + escapeHtml(kit.slug || '') + '" data-ti-full-import-name="' + escapeHtml(kit.name || kit.slug || '') + '" ' + (importBusy ? 'disabled' : '') + '>' + (importBusy ? 'Preparing...' : 'Install Template') + '</button>'
                        : (
                            '<a class="button wkit-button-tertiary" data-ti-full-import-remote="' + escapeHtml(kit.slug || '') + '" data-ti-full-import-name="' + escapeHtml(kit.name || kit.slug || '') + '" href="' + escapeHtml(GET_ALL_ACCESS_URL) + '" target="_blank" rel="noopener noreferrer"><span class="dashicons dashicons-awards margin-right-7"></span>Get all Access</a>'
                        ),
                    '</div>',
                    '</div>',
                    '</div>'
    ...
    function renderTemplateImporterBrowseDemoTab() {
        return [
            ...
            '<div class="wkit-radio-inputs" role="radiogroup" aria-label="Pricing filter">',
            '<label class="wkit-radio">',
            '<input type="radio" name="wkit-pricing-browse" value="all" data-ti-filter-browse="1" ' + ((state.templateImporter.browsePricingFilter || 'all') === 'all' ? 'checked' : '') + '>',
            '<span class="name">All</span>',
            '</label>',
            '<label class="wkit-radio">',
            '<input type="radio" name="wkit-pricing-browse" value="free" data-ti-filter-browse="1" ' + ((state.templateImporter.browsePricingFilter || 'all') === 'free' ? 'checked' : '') + '>',
            '<span class="name">Free</span>',
            '</label>',
            '<label class="wkit-radio">',
            '<input type="radio" name="wkit-pricing-browse" value="subscribe" data-ti-filter-browse="1" ' + ((state.templateImporter.browsePricingFilter || 'all') === 'subscribe' ? 'checked' : '') + '>',
            '<span class="name">Paid</span>',
            '</label>',
            '</div>',
            '</div>',
    ...
    function renderTemplateImporterView() {
        var importerBody = renderTemplateImporterImportedTab();
        var advancedImportEnabled = isAdvancedTemplateImportEnabled();
        if (state.templateImporter.activeTab === 'import-template' && advancedImportEnabled) {
            importerBody = renderTemplateImporterUploadTab();
        } else if (state.templateImporter.activeTab === 'browse-demo') {
            importerBody = renderTemplateImporterBrowseDemoTab();
        } else if (state.templateImporter.activeTab === 'full-import') {
            importerBody = renderTemplateImporterFullImportTab();
        } else if (state.templateImporter.activeTab === 'import-template' && !advancedImportEnabled) {
        ...
        return [
            ...
            '<div>',
            '<h5>Getting stuck ? Explore How to use Template Kit</h5>',
            '<a type="button" class="button wkit-button-tertiary" href="https://wiratheme.com/how-to-import-a-template-kit-in-wira-kit/" target="_blank" rel="noopener noreferrer"><span class="dashicons dashicons-book margin-right-7"></span>See Tutorials</a>',
            '</div>',
            '</div>',
            '</div>',
            '<div class="panel-column">',
    ...
    function render() {
        ...
        root.innerHTML = [
            ...
            '<li class="' + navWidgets + '"><a href="' + escapeHtml(data.widgetsUrl || '#') + '" class="accent-text" data-route="widgets">Widgets</a></li>',
    ...
    function setPath(path, push) {
        ...
        if (state.path === 'template-builder') {
            ...
            } else if (state.templateImporter.activeTab === 'imported-template') {
                ensureTemplateImporterLoaded();
            }
        }
    ...
    function importRemoteKitBySlug(slug) {
        ...
        postAjax(
            ...
            .then(function (result) {
                ...
                state.templateImporter.activeTab = 'imported-template';
    ...
    root.addEventListener('click', function (event) {
        ...
        if (tiTabEl) {
            event.preventDefault();
            var requestedTiTab = tiTabEl.getAttribute('data-ti-tab') || 'full-import';
            if (requestedTiTab === 'import-template' && !isAdvancedTemplateImportEnabled()) {
                requestedTiTab = 'full-import';
            }
            ...
            if (state.templateImporter.activeTab === 'imported-template') {
                ensureTemplateImporterLoaded();
            } else if (state.templateImporter.activeTab === 'browse-demo') {
                ensureRemoteBrowseLoaded();
            } else if (state.templateImporter.activeTab === 'full-import') {
