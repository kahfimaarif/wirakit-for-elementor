...
foreach ( $items as $item ) {
	...
	if ( $enable_category_tabs ) {
		...
		$raw_item_category = ! empty( $item['portfolio_category'] )
			? (string) $item['portfolio_category']
			: '';

		$category_list = array_map(
			'trim',
			explode( ',', $raw_item_category )
		);
		...
		foreach ( $category_list as $category_slug ) {
			...
			if ( $category_slug && isset( $category_map[ $category_slug ] ) ) {
				$item_categories[] = $category_slug;
			}
		}
	}

	$item_attr = ! empty( $item_categories )
		? implode( ' ', $item_categories )
		: 'uncategorized';
