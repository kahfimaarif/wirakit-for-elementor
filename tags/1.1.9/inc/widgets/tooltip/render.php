...
$tooltip_glow = isset( $settings['enable_tooltip_glow'] ) ? $settings['enable_tooltip_glow'] : '';

$this->add_render_attribute( 'tooltip_icon', 'class', 'wkit-tooltip-icon' );

if ( 'yes' === $tooltip_glow ) {
	$this->add_render_attribute( 'tooltip_icon', 'class', 'tooltip-glow' );
}
...
if ( '' !== $trigger_text || ! empty( $trigger_icon['value'] ) ) {
	...
	if ( $link_url ) {
		...
		if ( ! empty( $trigger_icon['value'] ) ) {
			echo '<span ' . $this->get_render_attribute_string( 'tooltip_icon' ) . '>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Safe: already escaped by Elementor
