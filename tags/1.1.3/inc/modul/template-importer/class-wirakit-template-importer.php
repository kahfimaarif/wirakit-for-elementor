...
if ( ! class_exists( 'Wirakit_Template_Importer' ) ) {
	...
	class Wirakit_Template_Importer {
		 ...
		 * @var string
		 */
		const GET_ALL_ACCESS_URL = 'https://wiratheme.com/wirakit/wira-extended/';
		...
		public function ajax_install_required_plugins() {
			...
			$auto_install_enabled = (bool) apply_filters( 'Wirakit_template_importer_enable_auto_install_required_plugins', false );
			if ( $auto_install_enabled && ! current_user_can( 'install_plugins' ) ) {
				wp_send_json_error( array( 'message' => __( 'Permission denied to install plugins.', 'wira-kit-for-elementor' ) ), 403 );
			}
			...
			if ( $auto_install_enabled ) {
				foreach ( $required_statuses as $plugin ) {
					...
					$plugin_id = '' !== $slug ? $slug : $file;

					if ( ! empty( $selected_plugins ) && ! in_array( $plugin_id, $selected_plugins, true ) ) {
						continue;
					}
					...
					if ( ! empty( $plugin['requires_purchase'] ) || ! empty( $plugin['auto_install_supported'] ) && false === $plugin['auto_install_supported'] ) {
						// Skip premium/non-supported installs (e.g. Elementor Pro). Status will be reported back to UI.
						continue;
					}

					if ( ! $this->is_allowed_required_plugin_slug( $slug ) ) {
						wp_send_json_error(
							array(
								'message' => sprintf(
									/* translators: %s: plugin slug */
									__( 'Plugin slug is not allowed for auto-install: %s', 'wira-kit-for-elementor' ),
									$slug ? $slug : __( '(empty)', 'wira-kit-for-elementor' )
								),
							),
							400
						);
					...
					$result = $this->install_or_activate_plugin( $slug, $file );
					if ( is_wp_error( $result ) ) {
						wp_send_json_error( array( 'message' => $result->get_error_message() ), 500 );
					}

					$processed_plugins[] = $slug ? $slug : $file;
				}
			}
			...
			wp_send_json_success(
				array(
					'message'  => $auto_install_enabled
						? __( 'Required plugins processed successfully.', 'wira-kit-for-elementor' )
						: ( empty( $unmet_plugins )
		...
		/**
		 * Install plugin if missing and activate when possible.
		 *
		 * @param string $slug Plugin slug.
		 * @param string $file Plugin file path.
		 * @return true|WP_Error
		 */
		private function install_or_activate_plugin( $slug, $file ) {
			...
			if ( '' === $resolved_file ) {
				if ( '' === (string) $slug ) {
					return new WP_Error( 'plugin_error', __( 'Missing plugin slug from manifest.', 'wira-kit-for-elementor' ) );
				}
				...
				if ( is_wp_error( $api ) ) {
					return new WP_Error( 'plugin_error', $api->get_error_message() );
				}
				...
				if ( is_wp_error( $install ) ) {
					return new WP_Error( 'plugin_error', $install->get_error_message() );
				}
				if ( is_wp_error( $skin->result ) ) {
					return new WP_Error( 'plugin_error', $skin->result->get_error_message() );
				}
				if ( $skin->get_errors()->has_errors() ) {
					return new WP_Error( 'plugin_error', $skin->get_error_messages() );
				}
				if ( null === $install ) {
					return new WP_Error( 'plugin_error', __( 'Unable to connect to filesystem.', 'wira-kit-for-elementor' ) );
				}
			...
			if ( '' === $resolved_file ) {
				return new WP_Error( 'plugin_error', __( 'Plugin installed but file entry was not found.', 'wira-kit-for-elementor' ) );
			}

			if ( is_plugin_inactive( $resolved_file ) ) {
				$activation = activate_plugin( $resolved_file, '', false, false );
				if ( is_wp_error( $activation ) ) {
					return new WP_Error( 'plugin_error', $activation->get_error_message() );
				}
			}
		...
		public function ajax_full_process_step() {
			...
			switch ( $step ) {
				...
				case 'assign-template-builder':
					$this->enable_all_template_builder_modules_for_import();
		...
		/**
		 * Ensure all Template Builder module toggles are enabled before assignments.
		 *
		 * This prevents sites from "missing" imported header/footer/single/etc. output
		 * due to module toggles being OFF (especially after updates).
		 *
		 * @return void
		 */
		private function enable_all_template_builder_modules_for_import() {
			if ( ! class_exists( 'Wirakit_Functions' ) ) {
				return;
			}

			$option_key = Wirakit_Functions::SETTINGS_OPTION_KEY;
			...
			$settings['modules']['template-builder']         = true;
			$settings['modules']['template-builder-header']  = true;
			$settings['modules']['template-builder-footer']  = true;
			$settings['modules']['template-builder-single']  = true;
			$settings['modules']['template-builder-archive'] = true;
			$settings['modules']['template-builder-search']  = true;
			$settings['modules']['template-builder-404']     = true;
		...
		private function get_required_plugins_status( $manifest ) {
			...
			foreach ( $required_plugins as $plugin ) {
				...
				$is_elementor_pro = ( 'elementor-pro' === sanitize_key( (string) $slug ) );
				...
				if ( $is_elementor_pro ) {
					// Elementor Pro is not hosted on wordpress.org, so we can't install it via Plugin_Upgrader.
					$purchase_url = apply_filters( 'Wirakit_template_importer_elementor_pro_purchase_url', 'https://elementor.com/pro/' );
				} elseif ( ! $installed && $slug && current_user_can( 'install_plugins' ) && $this->is_allowed_required_plugin_slug( $slug ) ) {
				...
				$statuses[] = array(
					...
					'installed' => $installed,
					'active'    => $active,
					'requires_purchase'   => (bool) $is_elementor_pro,
					'purchase_label'      => $is_elementor_pro ? __( 'Need additional purchase', 'wira-kit-for-elementor' ) : '',
					'purchase_url'        => $purchase_url,
					'auto_install_supported' => (bool) ( ! $is_elementor_pro ),
