...
if ( ! class_exists( 'Wirakit_Template_Builder_Frontend' ) ) {
	...
	class Wirakit_Template_Builder_Frontend {
		...
		public function setup_template_hooks() {
			...
			if ( ( ! class_exists( 'Wirakit_Functions' ) || Wirakit_Functions::is_module_enabled( 'template-builder-header' ) ) && $this->has_header_templates() ) {
				add_action( 'get_header', array( $this, 'override_theme_header_template' ), 99 );
				add_action( 'wkit_header', array( $this, 'render_header' ) );
			}

			if ( ( ! class_exists( 'Wirakit_Functions' ) || Wirakit_Functions::is_module_enabled( 'template-builder-footer' ) ) && $this->has_footer_templates() ) {
				add_action( 'get_footer', array( $this, 'override_theme_footer_template' ), 99 );
				add_action( 'wkit_footer', array( $this, 'render_footer' ) );
			}

			if ( ( ! class_exists( 'Wirakit_Functions' ) || Wirakit_Functions::is_module_enabled( 'template-builder-single' ) ) && $this->has_single_templates() ) {
				add_filter( 'template_include', array( $this, 'override_theme_single_template' ), 99 );
				add_action( 'wkit_single', array( $this, 'render_single' ) );
			}

			if ( ( ! class_exists( 'Wirakit_Functions' ) || Wirakit_Functions::is_module_enabled( 'template-builder-archive' ) ) && $this->has_archive_templates() ) {
			...
			if ( ( ! class_exists( 'Wirakit_Functions' ) || Wirakit_Functions::is_module_enabled( 'template-builder-search' ) ) && $this->has_search_templates() ) {
				add_filter( 'template_include', array( $this, 'override_theme_search_template' ), 101 );
				add_action( 'wkit_search', array( $this, 'render_search' ) );
			}

			if ( ( ! class_exists( 'Wirakit_Functions' ) || Wirakit_Functions::is_module_enabled( 'template-builder-404' ) ) && $this->has_not_found_templates() ) {
